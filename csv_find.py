...
name = Path(os.getcwd() + '/static/result.csv')
with open(name) as f:
...
def find_posl(text):   
    ...
    for i in range(len(data)):
        ...
        if 'Призначення грошової допомоги особі, яка проживає разом з особою з інвалідністю I або II групи внаслідок психічного розладу'.upper() in t.upper() and '00103' == row[1]:
            text = row[2]
        if 'Встановлення статусу учасника війни, видача посвідчення'.upper() in t.upper() and '00239' == row[1]:
            text = row[2]
    ...
    if len(find_text) == 0:
        ...
        for i in range(len(d)):
            ...
            if 'ВЗЯТТЯ НА ОБЛІК ДЛЯ ЗАБЕЗПЕЧЕННЯ САНАТОРНО-КУРОРТНИМ ЛІКУВАННЯМ (ПУТІВКАМИ) ветеранів війни та осІб, на яких поширюється дія законів УКРАЇНИ'.upper() in t.upper() and '00131' == row[0]:
                text = row[1]
            if 'ПРО ЗАТВЕРДЖЕННЯ ПОРЯДКУ ТА УМОВ ЗАБЕЗПЕЧЕННЯ СОЦІАЛЬНОЇ ТА ПРОФЕСІЙНОЇ АДАПТАЦІЇ ОСІБ, ЯКІ ЗВІЛЬНЯЮТЬСЯ АБО ЗВІЛЬНЕНІ З ВІЙСЬКОВОЇ СЛУЖБИ, З ЧИСЛА ВЕТЕРАНІВ ВІЙНИ, '.upper() in t.upper() and '01586' == row[0]:
                text = row[1]
